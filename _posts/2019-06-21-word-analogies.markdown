...
The [latent variable model](https://arxiv.org/abs/1502.03520) (TACL '16) was the first attempt to rigorously explain word analogy arithmetic. [Gittens et al.'s](https://www.aclweb.org/anthology/P17-1007) criticism of this theory is that it assumes that word vectors are known *a priori* and generated by randomly scaling vectors sampled from the unit sphere (or having properties consistent with this procedure). The latent variable model also relies on word vectors being isotropic, even though empirical work has found that [SGNS vectors are generally not](https://www.aclweb.org/anthology/D17-1308).

The [paraphrase model](https://www.aclweb.org/anthology/P17-1007) (ACL '17) contends that context words $C = \\{ c_1 , ..., c_m \\}$ are semantically equivalent to a single word $c$ if $p(w\|c_1,...,c_m) = p(w\|c)$. If word frequencies have a uniform distribution, the paraphrase of $C$ can be written as a sum of its context vectors; vector arithmetic corresponds to inclusion or exclusion in this set. However, the authors themselves acknowledge that this assumption is unrealistic: word frequencies follow a Zipf distribution, which is far from uniform. A [more recent paper](https://arxiv.org/pdf/1901.09813.pdf) (ICML '19) tries to work around this assumption but ignores the effect of negative sampling in SGNS, even incorrectly stating that it has a "detrimental effect" on word embeddings. The authors do not prove that their theory extends to GloVe either; they only conjecture that it does. Most importantly, neither work on the paraphrase model provides *any* sort of empirical evidence in support of the theory.
