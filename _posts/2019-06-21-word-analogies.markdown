...
The [latent variable model](https://arxiv.org/abs/1502.03520) was the first attempt to rigorously explain word analogy arithmetic. [Gittens et al.'s](https://www.aclweb.org/anthology/P17-1007) criticism of this theory is that it assumes that word vectors are known *a priori* and generated by randomly scaling vectors sampled from the unit sphere (or having properties consistent with this procedure). The latent variable model also relies on word vectors being isotropic, even though empirical work has found that [SGNS vectors are generally not](https://www.aclweb.org/anthology/D17-1308).

The [paraphrase model](https://www.aclweb.org/anthology/P17-1007) contends that context words $C = \\{ c_1 , ..., c_m \\}$ are semantically equivalent to a single word $c$ if $p(w\|c_1,...,c_m) = p(w\|c)$. If word frequencies have a uniform distribution, the paraphrase of $C$ can be written as a sum of its context vectors; vector arithmetic corresponds to inclusion in this set. However, the authors themselves acknowledge that this assumption is unrealistic: word frequencies follow a Zipf distribution, which is far from uniform. A [more recent work](https://arxiv.org/pdf/1901.09813.pdf) tries to work around this assumption but completely dismisses the effect of negative sampling in SGNS. Neither work provides any empirical evidence of the paraphrase model.
...
For example, for $$\vec{king} - \vec{man} + \vec{woman} = \vec{queen}$$ to hold in a noiseless SGNS or GloVe space, we would need the following to be satisfied by the training corpus:
