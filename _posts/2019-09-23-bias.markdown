...
> <span style="font-style: normal; letter-spacing: 0px; color: black"> For a set of word pairs $S$, let the bias subspace $B = \text{span}(\\{ \vec{x} - \vec{y}\, |\, (x,y) \in S\\})$. For every word $w \not \in S$, let the debiased word vector 
>
> $$\vec{w_d} \triangleq \vec{w} - \text{proj}_B \vec{w}$$
>
> For any embedding model that implicitly factorizes $M$ into a word matrix $W$ and a context matrix $C$, the reconstructed word-context matrix $W_d C^T = M_d$ is unbiased with respect to $S$. </span>
